...
Co60_regions=[cl.roi_cal(3090,1173.22,3120),
              cl.roi_cal(3505,1332.49,3530),
              cl.roi_cal(6550,2505.71,6570)]

Na22_regions=[cl.roi_cal(1370,511,1423),
              cl.roi_cal(3360,1274.53,3377),
              cl.roi_cal(4680,1785.53,4710)]

Cs137_regions=[cl.roi_cal(1751,661.65,1810)]

Co57_regions=[cl.roi_cal(375,122.06,393)]

rois_set=[cl.rois(Co60_regions+Na22_regions+Cs137_regions+Co57_regions)]

cal=cl.calibration(rois_set)

path='../../Data/Ge/CNF/'
Spre=['CAL_']
filenames_list,TotalSpectra,calrects=[],[],[]

for count in range(10):
    l=[]
    for fn in Spre:
        l.append(fn+str(count+1)+'.CNF')
...
for filenames in filenames_list:
    ...
    for index,Rois in enumerate(rois_set):
        ...
        Rois.regions[0].spectrum.set_cal(cal.a_0,cal.a_1)
...
measure=['PatronK.CNF','PatronTh.CNF','PatronU.CNF','Chucuri.CNF','Fondo42h.CNF']
CalMeasureIndex=[3,0,1,7,8]
...
Eb=0.5
fondoSP=sp.spectra(path+'Fondo42h.CNF')
fondoSP.set_cal(calprom[8][0],
                calprom[8][1])
fondoSP.rebining(Eb)
...
#plt.show()
#-------------------------------------
PU_regions=[Eroi.roi(1755,1764.49,1770)]
PTh_regions=[Eroi.roi(2607,2614.51,2620)]
PK_regions=[Eroi.roi(1455,1460.82,1465)]
Guayna_regions=[Eroi.roi(1755,1764.49,1770),
                Eroi.roi(2607,2614.51,2620),
                Eroi.roi(1455,1460.82,1465)]
fondo_regions=[Eroi.roi(1755,1764.49,1770),
                Eroi.roi(2607,2614.51,2620),
                Eroi.roi(1455,1460.82,1465)]
...
ddGuayanaTh_c=dGuayanaTh_c[0]*np.sqrt((dGuayanaTh_c[1]/dGuayanaTh_c[0])**2+(dIntTh_Guayana[1]/dIntTh_Guayana[0])**2+(m_RGK[1]/m_RGK[0])**2+(0.001/0.543)**2+(dIntTh[1]/dIntTh[0])**2)
...
ddGuayanaU_c=dGuayanaU_c[0]*np.sqrt((dGuayanaU_c[1]/dGuayanaU_c[0])**2+(dIntU_Guayana[1]/dIntU_Guayana[0])**2+(m_RGK[1]/m_RGK[0])**2+(0.001/0.543)**2+(dIntU[1]/dIntU[0])**2)


################################RESULTADOS##############################
K_Ge3x3,Th_Ge3x3,U_Ge3x3=[],[],[]

K_Ge3x3.append([dGuayanaPK_c[0],ddGuayanaPK_c,ActividadMK,dActividadMK])
Th_Ge3x3.append([dGuayanaTh_c[0],ddGuayanaTh_c,ActividadMTh,dActividadMTh])
U_Ge3x3.append([dGuayanaU_c[0],ddGuayanaU_c,ActividadMU,dActividadMU])
